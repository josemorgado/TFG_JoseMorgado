...
class VideoSerializer(serializers.ModelSerializer):
    ...
    # Validaciones principales del serializer
    def validate(self, attrs):
        # Se obtiene ct y object_id, considerando creación y actualización
        ...
        # Validar que el ContentType corresponda a un modelo real
        model_cls = ct.model_class() if ct else None
        if not model_cls:
            raise serializers.ValidationError({'content_type': 'ContentType inválido.'})

        # Validar que el objeto referenciado exista
        if object_id is None or not model_cls.objects.filter(pk=object_id).exists():
            raise serializers.ValidationError({'object_id': 'El objeto asociado no existe para ese ContentType.'})

        # Pre-chequeo del límite de videos (el modelo refuerza esta regla en clean())
    ...
    # Validación opcional: evita videos vacíos o string vacío
